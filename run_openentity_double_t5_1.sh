...
task=openentity
model_dir=./trained_models/t5_${task}_padtomaxlenF_bmf1_adaf_DOUBLET5_bs128
dt=$(date '+%d.%m.%Y_%H.%M.%S')
mkdir $model_dir
cp ./run_openentity_double_t5.sh $model_dir/run_openentity_double_t5.sh_$dt
...
\
                ...
                --task_name $task \
                --train_file ./data/${task}_double_t5_json/train.json \
                --validation_file ./data/${task}_double_t5_json/dev.json \
                --labels_file ./data/${task}_double_t5_json/labels.json \
...
\
		...
		--metric_for_best_model f1_micro \
                --test_file ./data/${task}_double_t5_json/test.json
