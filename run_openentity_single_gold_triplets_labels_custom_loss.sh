...
task=openentity
model_dir=./trained_models/t5_${task}_padtomaxlenF_wtripsinggoldlab_bmf1_adaf_bs64_custloss
dt=$(date '+%d.%m.%Y_%H.%M.%S')
mkdir $model_dir
cp ./run_openentity.sh $model_dir/run_openentity.sh_$dt
cp ./run_summarization_finetune_custom_loss.py $model_dir/run_summarization_finetune_custom_loss.py_$dt

python ./run_summarization_finetune_custom_loss.py \
		--use_triplets True \
